...
function createCustomLegend(traces, legendDiv, plotDiv) {
  ...
  traces.forEach((trace, i) => {
    ...
    let traceColor = trace.line?.color || trace.marker?.color || '#000000';
    
    item.innerHTML = `
      <svg width="20" height="20" style="margin-right: 5px;">
        <line x1="0" y1="10" x2="20" y2="10" stroke="${traceColor}" stroke-width="2"/>
      </svg>
      <span class="legend-text">${trace.name}</span>
    `;
    item.addEventListener('click', () => toggleTrace(i, plotDiv));
...
function toggleTrace(traceIndex, plotDiv) {
